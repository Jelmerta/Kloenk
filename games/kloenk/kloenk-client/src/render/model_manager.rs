...
pub struct ModelManager {
    required_models: HashMap<String, ModelDefinition>,
    loaded_models: HashMap<String, ModelDefinition>, // The loaded models are the actual usable models at this point in time -> This should converge with required_models after some time
...
impl ModelManager {
    ...
    pub fn add_required_model(&mut self, model: ModelDefinition) {
        self.required_models.insert(model.id.clone(), model);
    }
    ...
    fn update_ready(&mut self) {
        for model_id in self.required_models.keys() {
            if self.loaded_models.contains_key(model_id) {
                continue;
            }

            let model = self.required_models.get(model_id).unwrap();
    ...
    pub async fn new() -> ModelManager {
        ...
        model_manager.add_required_model(default_model_2d);
        ...
        model_manager.add_required_model(default_model_3d);

        model_manager.add_required_model(ModelLoader::load_colored_square_model(
            "black",
        ...
        model_manager.add_required_model(ModelLoader::make_preload_model(
            "shield",
            "CUBE",
            "shield.dds",
        ));
        ...
        for model in gozer_models {
            model_manager.add_required_model(model);
        }
        ...
        // References state of vertices_manager
        model_manager.added_vertices("SQUARE");
        model_manager.added_vertices("CUBE");

        // References state of texture manager
        model_manager.added_texture("white");

        // References state of color manager
        model_manager.added_color("white");
